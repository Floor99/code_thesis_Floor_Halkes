...
def build_geodataframes(
    df_lights: pd.DataFrame, df_nodes: pd.DataFrame
...
def find_nearest_lights(
    gdf_lights: gpd.GeoDataFrame, gdf_nodes: gpd.GeoDataFrame
) -> gpd.GeoDataFrame:
...
def apply_threshold(
    gdf_nodes: gpd.GeoDataFrame, threshold: float = 25
) -> gpd.GeoDataFrame:
...
def build_nodes_map(gdf_nodes: gpd.GeoDataFrame) -> pd.DataFrame:
...
def build_time_grid(df_meas: pd.DataFrame) -> pd.DataFrame:
...
def cross_join(df_nodes_map: pd.DataFrame, df_times: pd.DataFrame) -> pd.DataFrame:
...
def build_final_df(
    ...
    df.loc[df["has_light"] == 0, "wait_time"] = np.random.uniform(
        min_wait_time_no_light,
        max_wait_time_no_light,
        size=(df["has_light"] == 0).sum(),
    )

    if df["wait_time"].isnull().any():
        print(
            "Warning: Some wait times are still NaN after merging. This may indicate missing data for some lights at certain timestamps."
        )
...
def merge_timeseries_pipeline(
    meta_path: str, nodes_path: str, meas_path: str, threshold: float = 25
) -> pd.DataFrame:
    """
...
if __name__ == "__main__":
    ...
    df_final = merge_timeseries_pipeline(meta_path, nodes_path, meas_path, threshold)
