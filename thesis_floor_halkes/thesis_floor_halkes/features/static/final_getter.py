...
from thesis_floor_halkes.data_processing.create_subgraph import (
    consilidate_subgraph,
    consolidate_edges_to_single,
    create_subgraph_inside_helmond,
...
from thesis_floor_halkes.data_processing.merge_sub_timeseries import (
    merge_timeseries_pipeline,
)
...
def get_random_locations_with_traffic_lights(
    nodes_traffic_lights_path: str, seed=42, num_samples=16
):
    ...
    lijst_locations = [
        (row["lat"].item(), row["lon"].item()) for _, row in random_locations.iterrows()
    ]

    return lijst_locations


def get_random_timestamp(timeseries_path: str, seed=42):
...
def create_sub_network(
    ...
    G_sub, G_pt = create_subgraph_inside_helmond(
        nodes_helmond_path, lat, lon, dist=dist
    )
...
def get_timeseries_sub_network(
    node_features_path,
    meta_path,
    measurement_path,
    ...
    timeseries_subgraph = merge_timeseries_pipeline(
        meta_path, node_features_path, measurement_path, threshold
    )

    expanded_timeseries = expand_wait_times(
        timeseries_subgraph, num_peaks=2, amp_frac=0.1, sigma=1.0
    )
...
def get_timeseries_from_start_timestamp(
    expanded_timeseries, start_timestamp: pd.Timestamp, end_timestamp: pd.Timestamp
):
    """
...
def get_static_data_object_subgraph(
    ...
    timeseries = get_timeseries_from_start_timestamp(
        timeseries, start_timestamp=start_time, end_timestamp=end_time
    )
    ...
    edge_attr = torch.tensor(
        edge_features[["length", "maxspeed"]].values, dtype=torch.float, device=device
    )
    ...
    for network_id in os.listdir(root_dir):
        ...
        for _ in range(objects_per_network):
            try:
                ...
                # Construct filename
                filename = (
                    f"{network_id}_{static_data.start_node}_{static_data.end_node}.pt"
                )
...
class StaticDataObjectSet(InMemoryDataset):
    def __init__(
        ...
        super(StaticDataObjectSet, self).__init__(
            root, transform, pre_filter, pre_transform
        )
    ...
    def process(self):
        ...
        for file_path in file_paths:
            ...
            print(
                f"Loaded {data.graph_id} with start node {data.start_node} and end node {data.end_node}"
            )
